...
module.exports = getAllTasks = async (req, res) => {
  ...
  try {
    ...
    let json = await Promise.all(result.rows.map(async (r) => {
      ...
      return { taskId: r.task_id, name: r.name, username: user.username, status: r.status }
    }));
...
module.exports = getTask = async (req, res, taskId) => {
  ...
  try {
    ...
    let user = await getUserData(data.user_id)
    let json = { taskId: data.task_id, name: data.name, username: user.username, status: data.status }
    return res.status(200).json(json);
  } catch (error) {
    console.log(error);
    return res.status(500).send();
...
module.exports = doneTask = async (req, res, taskId) => {
  try {
    let result = await pool.query(
      "UPDATE Tasks SET status = $1, finished_on= to_timestamp($2) WHERE task_id= $3",
      [true, Date.now(), taskId]
    );
    return res.status(200).send();;
  } catch (error) {
    console.log(error);
